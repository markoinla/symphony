defmodule SymphonyElixir.OrchestratorStatusTest do
  ...
  test "orchestrator snapshot reflects last codex update and session id" do
    ...
    assert snapshot_entry.last_engine_timestamp == now

    assert snapshot_entry.last_engine_message == %{
             event: :notification,
  ...
  test "orchestrator snapshot tracks codex thread totals and app-server pid" do
    ...
    snapshot = GenServer.call(pid, :snapshot)
    assert %{running: [snapshot_entry]} = snapshot
    assert snapshot_entry.engine_pid == "4242"
    assert snapshot_entry.engine_input_tokens == 12
    assert snapshot_entry.engine_output_tokens == 4
    assert snapshot_entry.engine_total_tokens == 16
    assert snapshot_entry.turn_count == 1
    ...
    assert completed_state.engine_totals.input_tokens == 12
    assert completed_state.engine_totals.output_tokens == 4
    assert completed_state.engine_totals.total_tokens == 16
    assert is_integer(completed_state.engine_totals.seconds_running)
  end
  ...
  test "orchestrator snapshot tracks codex token-count cumulative usage payloads" do
    ...
    snapshot = GenServer.call(pid, :snapshot)
    assert %{running: [snapshot_entry]} = snapshot
    assert snapshot_entry.engine_input_tokens == 10
    assert snapshot_entry.engine_output_tokens == 5
    assert snapshot_entry.engine_total_tokens == 15

    send(pid, {:DOWN, process_ref, :process, self(), :normal})
    completed_state = :sys.get_state(pid)

    assert completed_state.engine_totals.input_tokens == 10
    assert completed_state.engine_totals.output_tokens == 5
    assert completed_state.engine_totals.total_tokens == 15
  end
  ...
  test "orchestrator token accounting prefers total_token_usage over last_token_usage in token_count payloads" do
    ...
    snapshot = GenServer.call(pid, :snapshot)
    assert %{running: [snapshot_entry]} = snapshot
    assert snapshot_entry.engine_input_tokens == 200
    assert snapshot_entry.engine_output_tokens == 100
    assert snapshot_entry.engine_total_tokens == 300
  end
  ...
  test "orchestrator token accounting accumulates monotonic thread token usage totals" do
    ...
    assert snapshot_entry.engine_input_tokens == 10
    assert snapshot_entry.engine_output_tokens == 4
    assert snapshot_entry.engine_total_tokens == 14
  end
  ...
  test "orchestrator token accounting ignores last_token_usage without cumulative totals" do
    ...
    snapshot = GenServer.call(pid, :snapshot)
    assert %{running: [snapshot_entry]} = snapshot
    assert snapshot_entry.engine_input_tokens == 0
    assert snapshot_entry.engine_output_tokens == 0
  ...
  test "orchestrator restarts stalled workers with retry backoff" do
    ...
    running_entry = %{
      ...
      last_engine_message: nil,
      last_engine_timestamp: stale_activity_at,
       ...
       %{
         running: [
           %{
             ...
             engine_pid: "4242",
             engine_total_tokens: 3_200,
             runtime_seconds: 75,
             turn_count: 7,
             last_engine_event: "turn_completed",
             last_engine_message: %{
               event: :notification,
               message: %{
                 "method" => "turn/completed",
  ...
  test "status dashboard humanizes full codex app-server event set" do
    ...
    Enum.each(event_cases, fn {method, payload, expected_fragment} ->
      ...
      humanized =
        StatusDashboard.humanize_engine_message(%{event: :notification, message: message})
  ...
  test "status dashboard humanizes dynamic tool wrapper events" do
    ...
    assert StatusDashboard.humanize_engine_message(completed) =~
             "dynamic tool call completed (linear_graphql)"

    assert StatusDashboard.humanize_engine_message(failed) =~
             "dynamic tool call failed (linear_graphql)"

    assert StatusDashboard.humanize_engine_message(unsupported) =~
  ...
  test "status dashboard unwraps nested codex payload envelopes" do
    ...
    assert StatusDashboard.humanize_engine_message(wrapped) =~ "turn completed"
    assert StatusDashboard.humanize_engine_message(wrapped) =~ "in 10"
  end
  ...
  test "status dashboard uses shell command line as exec command status text" do
    ...
    assert StatusDashboard.humanize_engine_message(message) == "git status --short"
  end
  ...
  test "status dashboard formats auto-approval updates from codex" do
    ...
    humanized = StatusDashboard.humanize_engine_message(message)
  ...
  test "status dashboard enriches wrapper reasoning and message streaming events with payload context" do
    ...
    assert StatusDashboard.humanize_engine_message(reasoning_message) =~
             "reasoning update: compare retry paths for Linear polling"

    assert StatusDashboard.humanize_engine_message(message_delta) =~
             "agent message streaming: writing workpad reconciliation update"

    assert StatusDashboard.humanize_engine_message(fallback_reasoning) == "reasoning update"
  end
