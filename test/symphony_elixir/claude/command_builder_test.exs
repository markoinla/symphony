defmodule SymphonyElixir.Claude.CommandBuilderTest do
  ...
  test "builds basic command with required flags" do
    cmd = CommandBuilder.build(default_config(), "/tmp/mcp.json")
  ...
  test "includes model when specified" do
    ...
    cmd = CommandBuilder.build(config, "/tmp/mcp.json")
  ...
  test "uses custom command path" do
    ...
    assert String.starts_with?(cmd, "/usr/local/bin/claude")
  end
end
