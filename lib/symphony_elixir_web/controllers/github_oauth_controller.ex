defmodule SymphonyElixirWeb.GitHubOAuthController do
  @moduledoc """
  ...
  defp authorize_direct(conn) do
    ...
    case OAuth.authorize_url(state, redirect_uri) do
      ...
      {:error, :missing_client_id} ->
        error_response(conn, 422, "missing_client_id", "GitHub OAuth client ID is not configured. Save it in settings first.")
    end
  end

  defp authorize_via_proxy(conn) do
    case ProxyClient.start_oauth_flow(:github) do
      {:ok, %{url: url, state: state, code_verifier: code_verifier}} ->
        :ok = ProxyClient.store_pending_flow(:github, state, code_verifier)
  ...
  def callback(conn, params) do
    ...
    with {:ok, code} <- fetch_param(params, "code"),
      ...
      do_redirect(conn, "/settings?github_oauth=success")
    else
      {:error, :missing_param} ->
        error = params["error"] || "missing_code"
        do_redirect(conn, "/settings?github_oauth=error&message=#{URI.encode(error)}")

      {:error, :invalid_state} ->
        do_redirect(conn, "/settings?github_oauth=error&message=invalid_state")

      {:error, reason} ->
        message = inspect(reason)
        do_redirect(conn, "/settings?github_oauth=error&message=#{URI.encode(message)}")
    end
  end
  ...
  def proxy_poll(conn, _params) do
    case ProxyClient.get_pending_flow(:github) do
      {:ok, state, code_verifier} ->
        case ProxyClient.poll_token(state, code_verifier) do
          {:ok, tokens} ->
            ProxyClient.clear_pending_flow(:github)
  ...
  defp build_redirect_uri(_conn) do
    ...
    |> Kernel.<>("/api/v1/oauth/github/callback")
  end
