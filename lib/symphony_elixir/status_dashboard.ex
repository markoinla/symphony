defmodule SymphonyElixir.StatusDashboard do
  ...
  defp snapshot_with_samples(token_samples, now_ms) do
    case snapshot_payload() do
      {:ok, %{running: running, retrying: retrying, engine_totals: engine_totals} = snapshot} ->
        total_tokens = Map.get(engine_totals, :total_tokens, 0)
  ...
  defp format_snapshot_content(snapshot_data, tps, terminal_columns_override \\ nil) do
    case snapshot_data do
      ...
      {:ok, %{running: running, retrying: retrying, engine_totals: engine_totals} = snapshot} ->
        ...
        engine_input_tokens = Map.get(engine_totals, :input_tokens, 0)
        engine_output_tokens = Map.get(engine_totals, :output_tokens, 0)
        engine_total_tokens = Map.get(engine_totals, :total_tokens, 0)
        engine_seconds_running = Map.get(engine_totals, :seconds_running, 0)
        ...
        ([
           ...
           colorize("│ Runtime: ", @ansi_bold) <>
             colorize(format_runtime_seconds(engine_seconds_running), @ansi_magenta),
           colorize("│ Tokens: ", @ansi_bold) <>
             colorize("in #{format_count(engine_input_tokens)}", @ansi_yellow) <>
             colorize(" | ", @ansi_gray) <>
             colorize("out #{format_count(engine_output_tokens)}", @ansi_yellow) <>
             colorize(" | ", @ansi_gray) <>
             colorize("total #{format_count(engine_total_tokens)}", @ansi_yellow),
  ...
  defp format_multi_workflow_snapshot(snapshot, tps, terminal_columns_override) do
    ...
    engine_totals = Map.get(snapshot, :engine_totals, %{})
    rate_limits = Map.get(snapshot, :rate_limits)
    engine_input_tokens = Map.get(engine_totals, :input_tokens, 0)
    engine_output_tokens = Map.get(engine_totals, :output_tokens, 0)
    engine_total_tokens = Map.get(engine_totals, :total_tokens, 0)
  ...
  defp linear_project_url(org_slug, project_slug)
       when is_binary(org_slug) and org_slug != "" do
    "https://linear.app/#{org_slug}/project/#{project_slug}/issues"
  end

  defp linear_project_url(_org_slug, project_slug) do
  ...
  defp format_workflow_rows(workflows) do
    Enum.flat_map(workflows, fn workflow ->
      ...
      tokens = Map.get(workflow, :engine_totals, %{}) |> Map.get(:total_tokens, 0)
      ...
      [workflow] ->
        {:ok,
         %{
           ...
           engine_totals: workflow.engine_totals,
  ...
  defp sum_engine_totals(workflows) do
    Enum.reduce(workflows, %{input_tokens: 0, output_tokens: 0, total_tokens: 0, seconds_running: 0}, fn workflow, totals ->
      engine_totals = Map.get(workflow, :engine_totals, %{})

      %{
        input_tokens: totals.input_tokens + Map.get(engine_totals, :input_tokens, 0),
        output_tokens: totals.output_tokens + Map.get(engine_totals, :output_tokens, 0),
        total_tokens: totals.total_tokens + Map.get(engine_totals, :total_tokens, 0),
        seconds_running: totals.seconds_running + Map.get(engine_totals, :seconds_running, 0)
      }
    end)
  end
  ...
  defp format_running_summary(running_entry, running_event_width) do
    ...
    pid = format_cell(running_entry.engine_pid || "n/a", @running_pid_width)
    total_tokens = running_entry.engine_total_tokens || 0
    ...
    event = running_entry.last_engine_event || "none"
    event_label = format_cell(summarize_message(running_entry.last_engine_message), running_event_width)
  ...
  defp snapshot_total_tokens({:ok, %{engine_totals: engine_totals}}) when is_map(engine_totals) do
    Map.get(engine_totals, :total_tokens, 0)
  end
  ...
  @doc false
  @spec humanize_engine_message(term()) :: String.t()
  def humanize_engine_message(nil), do: "no engine message yet"

  def humanize_engine_message(%{event: event, message: message}) do
    payload = unwrap_engine_message_payload(message)

    (humanize_engine_event(event, message, payload) || humanize_engine_payload(payload))
    |> truncate(140)
  end

  def humanize_engine_message(%{message: message}) do
    message
    |> unwrap_engine_message_payload()
    |> humanize_engine_payload()
    |> truncate(140)
  end

  def humanize_engine_message(message) do
    message
    |> unwrap_engine_message_payload()
    |> humanize_engine_payload()
    |> truncate(140)
  ...
  defp summarize_message(message), do: humanize_engine_message(message)

  defp humanize_engine_event(:session_started, _message, payload) do
  ...
  defp humanize_engine_event(:turn_input_required, _message, _payload), do: "turn blocked: waiting for user input"

  defp humanize_engine_event(:approval_auto_approved, message, payload) do
    method =
    ...
    base =
      if is_binary(method) do
        "#{humanize_engine_method(method, payload)} (auto-approved)"
      else
  ...
  defp humanize_engine_event(:tool_input_auto_answered, message, payload) do
    answer = map_value(message, ["answer", :answer])

    base =
      case humanize_engine_method("item/tool/requestUserInput", payload) do
  ...
  defp humanize_engine_event(:tool_call_completed, _message, payload),
    do: humanize_dynamic_tool_event("dynamic tool call completed", payload)

  defp humanize_engine_event(:tool_call_failed, _message, payload),
    do: humanize_dynamic_tool_event("dynamic tool call failed", payload)

  defp humanize_engine_event(:unsupported_tool_call, _message, payload),
    do: humanize_dynamic_tool_event("unsupported dynamic tool call rejected", payload)

  defp humanize_engine_event(:turn_ended_with_error, message, _payload), do: "turn ended with error: #{format_reason(message)}"
  defp humanize_engine_event(:startup_failed, message, _payload), do: "startup failed: #{format_reason(message)}"
  defp humanize_engine_event(:turn_failed, _message, payload), do: humanize_engine_method("turn/failed", payload)
  defp humanize_engine_event(:turn_cancelled, _message, _payload), do: "turn cancelled"
  defp humanize_engine_event(:malformed, _message, _payload), do: "malformed JSON event from engine"
  defp humanize_engine_event(_event, _message, _payload), do: nil

  defp unwrap_engine_message_payload(%{} = message) do
    cond do
  ...
  defp unwrap_engine_message_payload(message), do: message

  defp humanize_engine_payload(%{} = payload) do
    case map_value(payload, ["method", :method]) do
      method when is_binary(method) ->
        humanize_engine_method(method, payload)
  ...
  defp humanize_engine_payload(payload) when is_binary(payload) do
    payload
    |> String.replace("\n", " ")
    |> sanitize_ansi_and_control_bytes()
    |> String.trim()
  ...
  defp humanize_engine_method("thread/started", payload) do
  ...
  defp humanize_engine_method("turn/started", payload) do
  ...
  defp humanize_engine_method("turn/completed", payload) do
    status =
  ...
  defp humanize_engine_method("turn/failed", payload) do
    error_message =
  ...
  defp humanize_engine_method("turn/cancelled", _payload), do: "turn cancelled"

  defp humanize_engine_method("turn/diff/updated", payload) do
    diff =
  ...
  defp humanize_engine_method("turn/plan/updated", payload) do
    plan_entries =
  ...
  defp humanize_engine_method("thread/tokenUsage/updated", payload) do
    usage =
  ...
  defp humanize_engine_method("item/started", payload), do: humanize_item_lifecycle("started", payload)
  defp humanize_engine_method("item/completed", payload), do: humanize_item_lifecycle("completed", payload)

  defp humanize_engine_method("item/agentMessage/delta", payload),
    do: humanize_streaming_event("agent message streaming", payload)

  defp humanize_engine_method("item/plan/delta", payload),
    do: humanize_streaming_event("plan streaming", payload)

  defp humanize_engine_method("item/reasoning/summaryTextDelta", payload),
    do: humanize_streaming_event("reasoning summary streaming", payload)

  defp humanize_engine_method("item/reasoning/summaryPartAdded", payload),
    do: humanize_streaming_event("reasoning summary section added", payload)

  defp humanize_engine_method("item/reasoning/textDelta", payload),
    do: humanize_streaming_event("reasoning text streaming", payload)

  defp humanize_engine_method("item/commandExecution/outputDelta", payload),
    do: humanize_streaming_event("command output streaming", payload)

  defp humanize_engine_method("item/fileChange/outputDelta", payload),
    do: humanize_streaming_event("file change output streaming", payload)

  defp humanize_engine_method("item/commandExecution/requestApproval", payload) do
  ...
  defp humanize_engine_method("item/fileChange/requestApproval", payload) do
  ...
  defp humanize_engine_method("item/tool/requestUserInput", payload) do
    question =
  ...
  defp humanize_engine_method("tool/requestUserInput", payload),
    do: humanize_engine_method("item/tool/requestUserInput", payload)

  defp humanize_engine_method("account/updated", payload) do
    auth_mode =
  ...
  defp humanize_engine_method("account/rateLimits/updated", payload) do
    rate_limits =
  ...
  defp humanize_engine_method("account/chatgptAuthTokens/refresh", _payload), do: "account auth token refresh requested"

  defp humanize_engine_method("item/tool/call", payload) do
  ...
  defp humanize_engine_method(<<"codex/event/", suffix::binary>>, payload) do
    humanize_engine_wrapper_event(suffix, payload)
  end

  defp humanize_engine_method(method, payload) do
    msg_type =
  ...
  defp humanize_engine_wrapper_event("mcp_startup_update", payload) do
    server =
  ...
  defp humanize_engine_wrapper_event("mcp_startup_complete", _payload), do: "mcp startup complete"
  defp humanize_engine_wrapper_event("task_started", _payload), do: "task started"
  defp humanize_engine_wrapper_event("user_message", _payload), do: "user message received"

  defp humanize_engine_wrapper_event("item_started", payload) do
    case wrapper_payload_type(payload) do
      "token_count" -> humanize_engine_wrapper_event("token_count", payload)
      type when is_binary(type) -> "item started (#{humanize_item_type(type)})"
      _ -> "item started"
    end
  end

  defp humanize_engine_wrapper_event("item_completed", payload) do
    case wrapper_payload_type(payload) do
      "token_count" -> humanize_engine_wrapper_event("token_count", payload)
  ...
  defp humanize_engine_wrapper_event("agent_message_delta", payload),
    do: humanize_streaming_event("agent message streaming", payload)

  defp humanize_engine_wrapper_event("agent_message_content_delta", payload),
    do: humanize_streaming_event("agent message content streaming", payload)

  defp humanize_engine_wrapper_event("agent_reasoning_delta", payload),
    do: humanize_streaming_event("reasoning streaming", payload)

  defp humanize_engine_wrapper_event("reasoning_content_delta", payload),
    do: humanize_streaming_event("reasoning content streaming", payload)

  defp humanize_engine_wrapper_event("agent_reasoning_section_break", _payload), do: "reasoning section break"
  defp humanize_engine_wrapper_event("agent_reasoning", payload), do: humanize_reasoning_update(payload)
  defp humanize_engine_wrapper_event("turn_diff", _payload), do: "turn diff updated"
  defp humanize_engine_wrapper_event("exec_command_begin", payload), do: humanize_exec_command_begin(payload)
  defp humanize_engine_wrapper_event("exec_command_end", payload), do: humanize_exec_command_end(payload)
  defp humanize_engine_wrapper_event("exec_command_output_delta", _payload), do: "command output streaming"
  defp humanize_engine_wrapper_event("mcp_tool_call_begin", _payload), do: "mcp tool call started"
  defp humanize_engine_wrapper_event("mcp_tool_call_end", _payload), do: "mcp tool call completed"

  defp humanize_engine_wrapper_event("token_count", payload) do
  ...
  defp humanize_engine_wrapper_event(other, payload) do
    msg_type =
      map_path(payload, ["params", "msg", "type"]) ||
        map_path(payload, [:params, :msg, :type])
