defmodule SymphonyElixir.Orchestrator do
  ...
  def init(opts) do
    ...
    state = %State{
      ...
      project: project,
      engine_totals: @empty_engine_totals,
      engine_rate_limits: nil
    }
  ...
  def handle_info(
        {:engine_worker_update, issue_id, %{event: _, timestamp: _} = update},
        %{running: running} = state
      ) do
    case Map.get(running, issue_id) do
      nil ->
      ...
      running_entry ->
        {updated_running_entry, token_delta} = integrate_engine_update(running_entry, update)
  ...
  defp last_activity_timestamp(running_entry) when is_map(running_entry) do
    Map.get(running_entry, :last_engine_timestamp) || Map.get(running_entry, :started_at)
  end
    ...
    running =
      ...
      |> Enum.map(fn {issue_id, metadata} ->
        %{
          ...
          engine_pid: metadata.engine_pid,
          engine_input_tokens: metadata.engine_input_tokens,
          engine_output_tokens: metadata.engine_output_tokens,
          engine_total_tokens: metadata.engine_total_tokens,
          turn_count: Map.get(metadata, :turn_count, 0),
          started_at: metadata.started_at,
          last_engine_timestamp: metadata.last_engine_timestamp,
          last_engine_message: metadata.last_engine_message,
          last_engine_event: metadata.last_engine_event,
  ...
  defp integrate_engine_update(running_entry, %{event: event, timestamp: timestamp} = update) do
    token_delta = extract_token_delta(running_entry, update)
    engine_input_tokens = Map.get(running_entry, :engine_input_tokens, 0)
    engine_output_tokens = Map.get(running_entry, :engine_output_tokens, 0)
    engine_total_tokens = Map.get(running_entry, :engine_total_tokens, 0)
    engine_pid = Map.get(running_entry, :engine_pid)
    last_reported_input = Map.get(running_entry, :engine_last_reported_input_tokens, 0)
    last_reported_output = Map.get(running_entry, :engine_last_reported_output_tokens, 0)
    last_reported_total = Map.get(running_entry, :engine_last_reported_total_tokens, 0)
    turn_count = Map.get(running_entry, :turn_count, 0)

    {
      Map.merge(running_entry, %{
        last_engine_timestamp: timestamp,
        last_engine_message: summarize_engine_update(update),
        session_id: session_id_for_update(running_entry.session_id, update),
        last_engine_event: event,
        engine_pid: engine_pid_for_update(engine_pid, update),
        engine_input_tokens: engine_input_tokens + token_delta.input_tokens,
        engine_output_tokens: engine_output_tokens + token_delta.output_tokens,
        engine_total_tokens: engine_total_tokens + token_delta.total_tokens,
        engine_last_reported_input_tokens: max(last_reported_input, token_delta.input_reported),
        engine_last_reported_output_tokens: max(last_reported_output, token_delta.output_reported),
        engine_last_reported_total_tokens: max(last_reported_total, token_delta.total_reported),
  ...
  defp engine_pid_for_update(_existing, %{engine_pid: pid})
       when is_binary(pid),
       do: pid
  ...
  defp engine_pid_for_update(_existing, %{engine_pid: pid}) when is_list(pid),
    do: to_string(pid)

  defp engine_pid_for_update(existing, _update), do: existing
  ...
  defp summarize_engine_update(update) do
    %{
      event: update[:event],
  ...
  defp record_session_completion_totals(state, running_entry) when is_map(running_entry) do
    ...
    %{state | engine_totals: engine_totals}
  end

  defp record_session_completion_totals(state, _running_entry), do: state

  defp finalize_db_session(running_entry, reason) when is_map(running_entry) do
    session_id = running_entry_session_id(running_entry)
  ...
  defp apply_engine_token_delta(
         %{engine_totals: engine_totals} = state,
         %{input_tokens: input, output_tokens: output, total_tokens: total} = token_delta
       )
       when is_integer(input) and is_integer(output) and is_integer(total) do
    %{state | engine_totals: apply_token_delta(engine_totals, token_delta)}
  end

  defp apply_engine_token_delta(state, _token_delta), do: state

  defp apply_engine_rate_limits(%State{} = state, update) when is_map(update) do
    case extract_rate_limits(update) do
      %{} = rate_limits ->
        %{state | engine_rate_limits: rate_limits}
  ...
  defp apply_engine_rate_limits(state, _update), do: state

  defp apply_token_delta(engine_totals, token_delta) do
    input_tokens = Map.get(engine_totals, :input_tokens, 0) + token_delta.input_tokens
    output_tokens = Map.get(engine_totals, :output_tokens, 0) + token_delta.output_tokens
    total_tokens = Map.get(engine_totals, :total_tokens, 0) + token_delta.total_tokens

    seconds_running =
      Map.get(engine_totals, :seconds_running, 0) + Map.get(token_delta, :seconds_running, 0)
  ...
  defp extract_token_delta(running_entry, %{event: _, timestamp: _} = update) do
...

    {
      compute_token_delta(
        ...
        :input,
        usage,
        :engine_last_reported_input_tokens
      ),
      compute_token_delta(
        running_entry,
        :output,
        usage,
        :engine_last_reported_output_tokens
      ),
      compute_token_delta(
        running_entry,
        :total,
        usage,
        :engine_last_reported_total_tokens
      )
    }
    |> Tuple.to_list()
