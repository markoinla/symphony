defmodule SymphonyElixir.GitHub.OAuth do
  @moduledoc """
  GitHub OAuth2 authorization code flow.

  Handles building authorize URLs, exchanging authorization codes for tokens,
  and revoking access. Tokens are stored in the settings database with
  `github_oauth.*` keys.

  Unlike Linear, GitHub access tokens do not expire (unless the OAuth app is
  configured with token expiration enabled). When expiration is enabled the
  response includes `expires_in` and `refresh_token`; this module stores both
  and attempts a refresh when the token is within the refresh buffer.
  """

  require Logger

  alias SymphonyElixir.Store

  @authorize_url "https://github.com/login/oauth/authorize"
  @token_url "https://github.com/login/oauth/access_token"

  @refresh_buffer_seconds 300

  @type status :: :connected | :expired | :disconnected
  @type credentials_source :: :env | :store | :none

  @spec authorize_url(String.t(), String.t()) :: {:ok, String.t()} | {:error, :missing_client_id}
  def authorize_url(state, redirect_uri) when is_binary(state) and is_binary(redirect_uri) do
    case get_client_id() do
      nil ->
        {:error, :missing_client_id}

      client_id ->
        params =
          URI.encode_query(%{
            "client_id" => client_id,
            "redirect_uri" => redirect_uri,
            "scope" => "repo",
            "state" => state
          })

        {:ok, "#{@authorize_url}?#{params}"}
    end
  end

  @spec exchange_code(String.t(), String.t()) :: {:ok, map()} | {:error, term()}
  def exchange_code(code, redirect_uri) when is_binary(code) and is_binary(redirect_uri) do
    client_id = get_client_id()
    client_secret = get_client_secret()

    if is_nil(client_id) or is_nil(client_secret) do
      {:error, :missing_credentials}
    else
      body = %{
        "client_id" => client_id,
        "client_secret" => client_secret,
        "code" => code,
        "redirect_uri" => redirect_uri
      }

      case Req.post(@token_url,
             form: body,
             headers: [{"accept", "application/json"}],
             receive_timeout: 30_000
           ) do
        {:ok, %{status: 200, body: %{"access_token" => _} = token_data}} ->
          store_tokens(token_data)
          {:ok, token_data}

        {:ok, %{status: 200, body: %{"error" => error} = body}} ->
          Logger.error("GitHub OAuth token exchange failed error=#{error} body=#{inspect(body)}")
          {:error, {:token_exchange_failed, error}}

        {:ok, %{status: status, body: body}} ->
          Logger.error("GitHub OAuth token exchange failed status=#{status} body=#{inspect(body)}")
          {:error, {:token_exchange_failed, status}}

        {:error, reason} ->
          Logger.error("GitHub OAuth token exchange request failed: #{inspect(reason)}")
          {:error, {:token_exchange_request, reason}}
      end
    end
  end

  @spec credentials_source() :: credentials_source()
  def credentials_source do
    cond do
      Store.get_setting("github_oauth.client_id") != nil -> :store
      has_env_credentials?() -> :env
      true -> :none
    end
  end

  @spec revoke() :: :ok
  def revoke do
    delete_all_oauth_settings()
    :ok
  end

  @spec current_access_token() :: String.t() | nil
  def current_access_token do
    case Store.get_setting("github_oauth.access_token") do
      nil -> System.get_env("GITHUB_OAUTH_TOKEN")
      token -> maybe_refresh(token)
    end
  end

  @spec connection_status() :: {status(), String.t() | nil}
  def connection_status do
    case Store.get_setting("github_oauth.access_token") do
      nil ->
        if System.get_env("GITHUB_OAUTH_TOKEN") do
          {:connected, nil}
        else
          {:disconnected, nil}
        end

      _token ->
        expires_at = Store.get_setting("github_oauth.expires_at")

        cond do
          is_nil(expires_at) or not token_expired?(expires_at) -> {:connected, expires_at}
          refresh_expired_token_ok?() -> {:connected, Store.get_setting("github_oauth.expires_at")}
          true -> {:expired, expires_at}
        end
    end
  end

  @spec store_state(String.t()) :: :ok
  def store_state(state) when is_binary(state) do
    {:ok, _} = Store.put_setting("github_oauth.state", state)
    :ok
  end

  @spec validate_state(String.t()) :: :ok | {:error, :invalid_state}
  def validate_state(state) when is_binary(state) do
    case Store.get_setting("github_oauth.state") do
      ^state ->
        Store.delete_setting("github_oauth.state")
        :ok

      _ ->
        {:error, :invalid_state}
    end
  end

  @spec store_tokens(map()) :: :ok
  def store_tokens(%{"access_token" => access_token} = token_data) do
    {:ok, _} = Store.put_setting("github_oauth.access_token", access_token)

    if refresh = token_data["refresh_token"] do
      {:ok, _} = Store.put_setting("github_oauth.refresh_token", refresh)
    end

    if expires_in = token_data["expires_in"] do
      expires_at =
        DateTime.utc_now()
        |> DateTime.add(expires_in, :second)
        |> DateTime.truncate(:second)
        |> DateTime.to_iso8601()

      {:ok, _} = Store.put_setting("github_oauth.expires_at", expires_at)
    end

    :ok
  end

  defp refresh_expired_token_ok? do
    match?({:ok, _}, refresh_token())
  end

  defp maybe_refresh(token) do
    if token_needs_refresh?() do
      case refresh_token() do
        {:ok, %{"access_token" => refreshed}} -> refreshed
        {:error, _reason} -> token
      end
    else
      token
    end
  end

  defp token_needs_refresh? do
    case Store.get_setting("github_oauth.expires_at") do
      nil -> false
      expires_at_str -> within_refresh_buffer?(expires_at_str)
    end
  end

  defp within_refresh_buffer?(expires_at_str) do
    case DateTime.from_iso8601(expires_at_str) do
      {:ok, expires_at, _} ->
        DateTime.diff(expires_at, DateTime.utc_now(), :second) < @refresh_buffer_seconds

      _ ->
        false
    end
  end

  @spec refresh_token() :: {:ok, map()} | {:error, term()}
  def refresh_token do
    refresh_token_val = Store.get_setting("github_oauth.refresh_token")
    client_id = get_client_id()
    client_secret = get_client_secret()

    if is_nil(refresh_token_val) or is_nil(client_id) or is_nil(client_secret) do
      {:error, :missing_refresh_token}
    else
      body = %{
        "grant_type" => "refresh_token",
        "refresh_token" => refresh_token_val,
        "client_id" => client_id,
        "client_secret" => client_secret
      }

      case Req.post(@token_url,
             form: body,
             headers: [{"accept", "application/json"}],
             receive_timeout: 30_000
           ) do
        {:ok, %{status: 200, body: %{"access_token" => _} = token_data}} ->
          store_tokens(token_data)
          {:ok, token_data}

        {:ok, %{status: status, body: body}} ->
          Logger.error("GitHub OAuth token refresh failed status=#{status} body=#{inspect(body)}")
          {:error, {:token_refresh_failed, status}}

        {:error, reason} ->
          Logger.error("GitHub OAuth token refresh request failed: #{inspect(reason)}")
          {:error, {:token_refresh_request, reason}}
      end
    end
  end

  defp token_expired?(expires_at_str) do
    case DateTime.from_iso8601(expires_at_str) do
      {:ok, expires_at, _} -> DateTime.compare(expires_at, DateTime.utc_now()) == :lt
      _ -> false
    end
  end

  defp delete_all_oauth_settings do
    ~w(
      github_oauth.access_token
      github_oauth.refresh_token
      github_oauth.expires_at
      github_oauth.state
    )
    |> Enum.each(&Store.delete_setting/1)
  end

  defp has_env_credentials? do
    env_id = System.get_env("GITHUB_OAUTH_CLIENT_ID")
    env_secret = System.get_env("GITHUB_OAUTH_CLIENT_SECRET")
    is_binary(env_id) and env_id != "" and is_binary(env_secret) and env_secret != ""
  end

  defp get_client_id do
    Store.get_setting("github_oauth.client_id") || System.get_env("GITHUB_OAUTH_CLIENT_ID")
  end

  defp get_client_secret do
    Store.get_setting("github_oauth.client_secret") || System.get_env("GITHUB_OAUTH_CLIENT_SECRET")
  end
end
