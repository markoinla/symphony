...
function LinearIssueBadge({ identifier }: { identifier: string }) {
  return (
    <Button
      className="h-auto shrink-0 rounded bg-th-accent-muted px-1.5 py-0.5 text-[11px] font-medium text-th-accent shadow-none hover:bg-th-accent-muted hover:opacity-80"
      onClick={(e) => {
        e.stopPropagation()
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="truncate text-sm font-medium text-th-text-1">{entry.issue_identifier}</span>
            <LinearIssueBadge identifier={entry.issue_identifier} />
            <Badge variant="retrying">Retry {entry.attempt}</Badge>
          </div>
          <p className="mt-2 line-clamp-2 text-[13px] leading-5 text-th-text-3">
...
const ERROR_CATEGORY_CONFIG: Record<string, { variant: 'destructive' | 'retrying' | 'live' | 'secondary'; label: string; className?: string }> = {
  infra: { variant: 'destructive', label: 'Infra' },
  agent: { variant: 'retrying', label: 'Agent' },
  config: { variant: 'secondary', label: 'Config', className: 'bg-amber-500/15 text-amber-600 dark:text-amber-400' },
  timeout: { variant: 'live', label: 'Timeout' },
  shutdown: { variant: 'secondary', label: 'Shutdown' },
}
