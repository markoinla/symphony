...
name: commit
description:
  Create a well-formed git commit from current changes using conversation history
  for rationale and summary; use when asked to commit, prepare a commit message,
  or finalize staged work.
---
...
- Conversation history for intent and rationale.
...
9. Append a `Co-authored-by` trailer using
   `Co-Authored-By: Claude <noreply@anthropic.com>` unless the user explicitly
...
- A single commit created with `git commit` whose message reflects the
  conversation.
