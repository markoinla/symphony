...
name: debug
description:
  Investigate stuck runs and execution failures by tracing Symphony and engine
...
- Correlate Linear issue identity to an engine session quickly.
...
- Primary runtime log: `log/symphony.log`
  ...
  - Includes orchestrator, agent runner, and engine lifecycle logs.
...
4. Trace that `session_id` across start, stream, completion/failure, and stall
   handling logs.
5. Decide class of failure: timeout/stall, engine startup failure, turn
...
## Reading Engine Session Logs

In Symphony, engine session diagnostics are emitted into `log/symphony.log` and
