...
# Wait for review feedback: reviews arrive as issue comments that start
...
- Automated review jobs retry on failure and are non-blocking; use the presence
  of review issue comments (not job status) as the signal that review feedback
  is available.
...
- Only request a new review when you need a rerun (e.g., after new commits). Do
  not request one without changes since the last review.
  - Before requesting a new review, re-run the land watcher and ensure there are
    zero outstanding review comments (all have `[claude]` inline replies).
